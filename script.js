        ...
        fetch(`https://order-tracking-widgetsts.onrender.com/order-status?order_name=${orderNumber}`)
            ...
            .then(data => {
                ...
                if (data.status === 'ready') {
                    ...
                    frame.querySelector('p').innerHTML = `Su producto ha sido enviado. El pedido se ha enviado mediante ${data.info.carrier} y el número de envío es <a href="https://s.correosexpress.com/SeguimientoSinCP/search?n=${data.info.tracking}" target="_blank">${data.info.tracking}</a>.`;
                    ...
                    frame.querySelector('p').innerHTML = `El producto ${data.info.producto} está en preventa y no se va a enviar hasta ${data.info.fecha}. Si quieres un pedido parcial (4,99€), escribe a <a href="mailto:tienda@thestoreteam.com">tienda@thestoreteam.com</a>.`;
                    frame.classList.add('visible');
                } else {
